...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://counselstack.io/for/${slug}` },
  };
}
    ...
    <>
      <BreadcrumbSchema
        items={[
          ...
          { name: "By Practice Type", url: "https://counselstack.io/for" },
          { name: audience.title, url: `https://counselstack.io/for/${slug}` },
        ]}
      />
      <ItemListSchema
        name={audience.heroHeadline}
        url={`https://counselstack.io/for/${slug}`}
      ...
      <main className="pt-16">
        ...
        <section className="bg-gradient-to-b from-brand-50 to-white pt-12 pb-8">
          <div className="mx-auto max-w-4xl px-6">
            <nav className="mb-6 text-sm text-slate-500">
              ...
              <Link href="/for" className="hover:text-brand-700">By Practice Type</Link>
              <span className="mx-2">/</span>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-4 grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <ul className="space-y-1">
                            {rec.cons.map((con, i) => (
                              <li key={i} className="flex items-start gap-2 text-xs text-slate-600">
                                <span className="text-orange-400 shrink-0">&bull;</span>{con}
                              </li>
                            ))}
                          </ul>
                        </div>
                      ...
                      <div className="mt-4 flex items-center gap-3">
                        <a href={rec.affiliateUrl} target="_blank" rel="noopener noreferrer sponsored" className="inline-flex items-center gap-2 rounded-lg bg-brand-700 px-4 py-2 text-sm font-medium text-white hover:bg-brand-800 transition-colors">
                          Try {rec.name} Free <ArrowRight className="h-3 w-3" />
                        </a>
        ...
        <section className="py-12 bg-slate-50">
          <div className="mx-auto max-w-4xl px-6">
            <h2 className="text-xl font-bold text-slate-900 mb-4">Browse Other Practice Types</h2>
